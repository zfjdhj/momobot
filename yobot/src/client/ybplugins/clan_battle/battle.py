...
from ..ybdata import Clan_challenge, Clan_group, Clan_member, Clan_subscribe, User
from .exception import ClanBattleError, GroupError, GroupNotExist, InputError, UserError, UserNotInGroup
...
class ClanBattle:
    ...
    def __init__(self, glo_setting: Dict[str, Any], bot_api: Api, *args, **kwargs):
        ...
        User.update({User.authority_group: 100}).where(User.authority_group == 1).execute()
        User.update({User.authority_group: 1}).where(User.qqid.in_(self.setting["super-admin"])).execute()
    ...
    def _get_nickname_by_qqid(self, qqid) -> Union[str, None]:
        ...
        if user.nickname is None:
            asyncio.ensure_future(
                self._update_user_nickname_async(
                    qqid=qqid,
                    group_id=None,
                )
            )
    ...
    def _get_group_previous_challenge(self, group: Clan_group):
        ...
        query = Clan_challenge.select().where(
            Clan_challenge.cid
            == Clan_challenge_alias.select(peewee.fn.MAX(Clan_challenge_alias.cid)).where(
    ...
    async def _fetch_member_list_async(self, group_id):
        try:
            ...
            asyncio.ensure_future(self.api.send_group_msg(group_id=group_id, message="获取群成员错误，这可能是缓存问题，请重启酷Q后再试"))
            return []
    ...
    def _boss_data_dict(self, group: Clan_group) -> Dict[str, Any]:
        return {
            ...
            "full_health": (
                self.bossinfo[group.game_server][self._level_by_cycle(group.boss_cycle, game_server=group.game_server)][
                    group.boss_num - 1
                ]
            ),
        }
    ...
    def drop_member(self, group_id: Groupid, member_list: List[QQid]):
        ...
        """
        delete_count = (
            Clan_member.delete().where(Clan_member.group_id == group_id, Clan_member.qqid.in_(member_list)).execute()
        )
    ...
    def boss_status_summary(self, group_id: Groupid) -> str:
        ...
        if group.challenging_member_qq_id is not None:
            ...
            boss_summary += "\n{}{}boss".format(
                escape(self._get_nickname_by_qqid(group.challenging_member_qq_id) or group.challenging_member_qq_id),
                action,
            )
    ...
    def challenge(
        ...
        *,
        ...
        is_member = Clan_member.get_or_none(group_id=group_id, qqid=qqid)
        if not is_member:
            raise UserNotInGroup
        d, t = pcr_datetime(area=group.game_server)
        if previous_day:
            today_count = (
                Clan_challenge.select()
                .where(
        ...
        finished = sum(bool(c.boss_health_ramain or c.is_continue) for c in challenges)
        if finished >= 3:
            if previous_day:
                raise InputError("昨日上报次数已达到3次")
            raise InputError("今日上报次数已达到3次")
        is_continue = challenges and challenges[-1].boss_health_ramain == 0 and not challenges[-1].is_continue
        ...
        if defeat:
            ...
            group.boss_health = self.bossinfo[group.game_server][
                self._level_by_cycle(group.boss_cycle, game_server=group.game_server)
            ][group.boss_num - 1]
        else:
        ...
        Clan_subscribe.delete().where(
            ...
            (Clan_subscribe.subscribe_item == 0) | (Clan_subscribe.subscribe_item == challenge.boss_num),
        ).execute()
        ...
        if defeat:
            ...
            msg = "{}对boss造成了{:,}点伤害\n（今日第{}刀，{}）".format(nik, damage, finished + 1, "剩余刀" if is_continue else "完整刀")
        status = BossStatus(
            group.boss_cycle,
            group.boss_num,
            group.boss_health,
        ...
        self._boss_status[group_id].set_result((self._boss_data_dict(group), msg))
        self._boss_status[group_id] = asyncio.get_event_loop().create_future()
    ...
    def undo(self, group_id: Groupid, qqid: QQid) -> BossStatus:
        ...
        group.boss_health = last_challenge.boss_health_ramain + last_challenge.challenge_damage
        ...
        self._boss_status[group_id].set_result((self._boss_data_dict(group), status.info))
        self._boss_status[group_id] = asyncio.get_event_loop().create_future()
        return status
    ...
    def modify(self, group_id: Groupid, cycle=None, boss_num=None, boss_health=None):
        ...
        if boss_health is None:
            boss_health = self.bossinfo[group.game_server][
                self._level_by_cycle(group.boss_cycle, game_server=group.game_server)
            ][group.boss_num - 1]
    ...
    def get_data_slot_record_count(self, group_id: Groupid):
        ...
        for c in (
            ...
            .where(Clan_challenge.gid == group_id)
            .group_by(
                Clan_challenge.bid,
            )
        ):
    ...
    def send_remind(self, group_id: Groupid, member_list: List[QQid], sender: QQid, send_private_msg: bool = False):
        """
    ...
    def notify_subscribe(self, group_id: Groupid, boss_num=None, send_private_msg=False):
        ...
        for subscribe in (
            ...
            .where(
                Clan_subscribe.gid == group_id,
                (Clan_subscribe.subscribe_item == boss_num) | (Clan_subscribe.subscribe_item == 0),
            )
    ...
    def apply_for_challenge(
        ...
        *,
        ...
        nik = escape(nik)
        info = f"{nik}已开始挑战boss" if appli_type == 1 else f"{nik}锁定了boss\n留言：{escape(extra_msg)}"
        status = BossStatus(
            group.boss_cycle,
            group.boss_num,
            group.boss_health,
    ...
    def cancel_application(self, group_id: Groupid, qqid: QQid) -> BossStatus:
        ...
        if (group.challenging_member_qq_id != qqid) and (user.authority_group >= 100):
            challenge_duration = int(time.time()) - group.challenging_start_time
            ...
            if (not is_challenge) or (challenge_duration < 180):
                ...
                msg = f"失败，{nik}在{challenge_duration}秒前" + (
                    "开始挑战boss" if is_challenge else ("锁定了boss\n留言：" + escape(group.challenging_comment))
                )
                raise GroupError(msg)
        group.challenging_member_qq_id = None
        group.save()
    ...
    def get_report(
        ...
        if isinstance(battle_id, str):
            if battle_id == "all":
                ...
                raise InputError(f'unexceptd value "{battle_id}" for battle_id')
        else:
        ...
        for c in Clan_challenge.select().where(*expressions):
            report.append(
                {
                    "battle_id": c.bid,
                    "qqid": c.qqid,
    ...
    def get_clan_daily_challenge_counts(
        self,
        group_id: Groupid,
        pcrdate: Optional[Pcr_date] = None,
    ...
    def execute(self, match_num, ctx):
        ...
        if match_num == 1:  # 创建
            ...
            return "公会创建成功，请登录后台查看，" "公会战成员请发送“加入公会”，" "或发送“加入全部成员”"
        elif match_num == 2:  # 加入
            if cmd == "加入全部成员":
            ...
            try:
                ...
                return str(e)
            finished = full_challenge_count + continued_challenge_count + continued_tailing_challenge_count
            unfinished = tailing_challenge_count - continued_challenge_count - continued_tailing_challenge_count
            return "今日进度\n\n" f"已完成出刀：{finished}\n" f"未完成的尾刀：{unfinished}\n" f"未开始的出刀：{90-finished-unfinished}"
        elif match_num == 4:  # 报刀
            match = re.match(r"^报刀 ?(\d+)([Ww万Kk千])? *(?:\[CQ:at,qq=(\d+)\])? *(昨[日天])? *(?:[\:：](.*))?$", cmd)
            if not match:
                return
            unit = {
                "W": 10000,
            ...
            try:
                boss_status = self.challenge(
                    group_id, user_id, False, damage, behalf, extra_msg=extra_msg, previous_day=previous_day
                )
            except ClanBattleError as e:
                _logger.info("群聊 失败 {} {} {}".format(user_id, group_id, cmd))
                return str(e)
            ...
            match = re.match(r"^尾刀 ?(?:\[CQ:at,qq=(\d+)\])? *(昨[日天])? *(?:[\:：](.*))?$", cmd)
            if not match:
                return
            ...
            try:
                boss_status = self.challenge(
                    group_id, user_id, True, None, behalf, extra_msg=extra_msg, previous_day=previous_day
                )
            except ClanBattleError as e:
                _logger.info("群聊 失败 {} {} {}".format(user_id, group_id, cmd))
                return str(e)
            ...
            if len(cmd) != 2:
                return
            url = urljoin(
                self.setting["public_address"], "{}clan/{}/".format(self.setting["public_basepath"], group_id)
            )
            return "请登录面板操作：" + url
        elif match_num == 8:  # 选择
            if len(cmd) != 2:
                return
            url = urljoin(
                self.setting["public_address"], "{}clan/{}/setting/".format(self.setting["public_basepath"], group_id)
            )
            return "请登录面板操作：" + url
        elif match_num == 9:  # 报告
            today = int(time.time() + 3600 * 3) // 86400
            ...
            if len(cmd) != 2:
                return
            url = urljoin(
                self.setting["public_address"], "{}clan/{}/progress/".format(self.setting["public_basepath"], group_id)
            )
            return "请在面板中查看：" + url + "\n" + reply
            ...
            if cmd == "申请出刀":
                ...
                if isinstance(extra_msg, str):
                    ...
                    return
            try:
                boss_status = self.apply_for_challenge(group_id, user_id, extra_msg=extra_msg, appli_type=appli_type)
            except ClanBattleError as e:
                _logger.info("群聊 失败 {} {} {}".format(user_id, group_id, cmd))
                return str(e)
            _logger.info("群聊 成功 {} {} {}".format(user_id, group_id, cmd))
            ...
            match = re.match(r"^清空(?:预约)?([1-5])? *$", cmd)
            if match:
                if ctx["sender"]["role"] == "member":
                    ...
                    if counts == 0:
                        _logger.info("群聊 失败 {} {}".format(group_id, cmd))
                        return "该boss的预约表为空"
                    _logger.info("群聊 成功 {} {}".format(user_id, cmd))
                    return event
    ...
    def register_routes(self, app: Quart):
        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/"), methods=["GET"])
        async def yobot_clan(group_id):
            ...
            is_member = Clan_member.get_or_none(group_id=group_id, qqid=session["yobot_user"])
        ...
        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/subscribers/"), methods=["GET"])
        ...
        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/api/"), methods=["POST"])
        async def yobot_clan_api(group_id):
            ...
            if "yobot_user" not in session:
                ...
                is_member = Clan_member.get_or_none(group_id=group_id, qqid=user_id)
                if not is_member and user.authority_group >= 10:
                    return jsonify(
                        code=11,
                ...
                if action == "get_member_list":
                    ...
                    try:
                        bossData, notice = await asyncio.wait_for(
                            asyncio.shield(self._boss_status[group_id]), timeout=30
                        )
                        return jsonify(
                            code=0,
                            bossData=bossData,
                    ...
                    try:
                        status = self.cancel_application(group_id, user_id)
                    except ClanBattleError as e:
                        _logger.info("网页 失败 {} {} {}".format(user_id, group_id, action))
                        return jsonify(
                            code=10,
                    ...
                    try:
                        self.save_slot(group_id, user_id, todaystatus=todaystatus)
                    except ClanBattleError as e:
                        _logger.info("网页 失败 {} {} {}".format(user_id, group_id, action))
                        return jsonify(
                            code=10,
                    ...
                    if group.notification & 0x200:
                        asyncio.ensure_future(
                            self.api.send_group_msg(
                                group_id=group_id,
                                message=(self._get_nickname_by_qqid(user_id) + f"已{sw}SL"),
                            )
                        )
                    return jsonify(code=0, notice=f"已{sw}SL")
                elif action == "get_subscribers":
                    subscribers = self.get_subscribe_list(group_id)
                    return jsonify(code=0, group_name=group.group_name, subscribers=subscribers)
                    ...
                    if counts == 0:
                        _logger.info("网页 失败 {} {} {}".format(user_id, group_id, action))
                        return jsonify(code=0, notice=("没有预约记录" if boss_num else "没有挂树记录"))
                    _logger.info("网页 成功 {} {} {}".format(user_id, group_id, action))
                    if boss_num == 0:
                        notice = "取消挂树成功"
                        if group.notification & 0x20:
                            asyncio.ensure_future(
                                self.api.send_group_msg(
                                    group_id=group_id,
                                    message="{}已取消挂树".format(user.nickname),
                                )
                            )
                    else:
                        notice = "取消预约成功"
                    ...
                    self.send_remind(group_id, payload["memberlist"], sender=sender, send_private_msg=private)
                    return jsonify(
                        code=0,
                        notice="发送成功",
                    )
        ...
        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/my/"), methods=["GET"])
        ...
        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/<int:qqid>/"), methods=["GET"])
        ...
        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/setting/"), methods=["GET"])
        async def yobot_clan_setting(group_id):
            ...
            is_member = Clan_member.get_or_none(group_id=group_id, qqid=session["yobot_user"])
            if not is_member:
                return await render_template("unauthorized.html", limit="本公会成员", uath="无")
            if user.authority_group >= 100:
                return await render_template("unauthorized.html", limit="公会战管理员", uath="成员")
            return await render_template("clan/setting.html")

        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/setting/api/"), methods=["POST"])
        ...
        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/statistics/"), methods=["GET"])
        ...
        @app.route(
            urljoin(self.setting["public_basepath"], "clan/<int:group_id>/statistics/<int:sid>/"), methods=["GET"]
        )
        ...
        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/statistics/api/"), methods=["GET"])
        ...
        @app.route(urljoin(self.setting["public_basepath"], "clan/<int:group_id>/progress/"), methods=["GET"])
