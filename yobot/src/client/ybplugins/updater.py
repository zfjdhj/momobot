...
class Updater:
    ...
    async def send_reply(
        self, context: Dict[str, Any], message: Union[str, Dict[str, Any], List[Dict[str, Any]]], **kwargs
    ...
    async def windows_update_async(self, force: bool = False, test_ver: int = 0):
        ...
        shutil.move(
            os.path.join(self.path, "temp", verstr, "yobot.exe"), os.path.join(self.working_path, "yobot.new.exe")
        )
        cmd = """
            cd '{}'
            ...
            """.format(
            self.working_path, os.getpid()
        )
        with open(os.path.join(self.path, "update.bat"), "w") as f:
            f.write(cmd)
        os.system("powershell Start-Process -FilePath '{}'".format(os.path.join(self.path, "update.bat")))
        sys.exit()
    ...
    async def windows_update_git_async(self, force: bool = False, test_ver: int = 0):
        ...
        """.format(
            self.path, os.getpid(), os.path.join(self.working_path, "main.py")
        )
        with open(os.path.join(git_dir, "update.bat"), "w") as f:
            f.write(cmd)
        os.system("powershell Start-Process -FilePath '{}'".format(os.path.join(git_dir, "update.bat")))
        sys.exit()
            ...
            if platform.system() == "Windows":
                if self.evn == "exe":
                    cmd = """
                        ...
                        """.format(
                        self_pid, os.path.join(self.working_path, "yobot.exe")
                    )
                    ...
                    cmd = """
                        ...
                        powershell Start-Process -FilePath "python.exe" -ArgumentList '{}'
                        """.format(
                        self_pid, os.path.join(self.working_path, "main.py")
                    )
                with open(os.path.join(self.path, "restart.bat"), "w") as f:
                    f.write(cmd)
                os.system("powershell Start-Process -FilePath '{}'".format(os.path.join(self.path, "restart.bat")))
                sys.exit(10)
            else:
                open(".YOBOT_RESTART", "w").close()
                sys.exit(10)
    ...
    # def jobs_restart(self) -> Iterable[Tuple[CronTrigger, Callable[[], List[Dict[str, Any]]]]]:
    #     # 自动重启
    #     # time = "03:30"
    #     # hour, minute = time.split(":")
    #     # trigger = CronTrigger(hour=hour, minute=minute)
    #     trigger = CronTrigger(minute="*/1")
    #     job = (trigger, self.restart())
    #     return (job,)
...
def get_version(base_version: str, base_commit: int) -> dict:
    ...
    if os.path.exists("/.dockerenv"):
        return {"run-as": "docker", "commited": False, "ver_name": f"yobot{base_version} on Docker"}
    try:
        with os.popen("git diff HEAD --stat") as r:
            text = r.read()
        if text != "":
            return {"run-as": "python", "commited": False, "ver_name": "yobot{}源码版\n存在未提交的修改".format(base_version)}
    except Exception as e:
        return {"run-as": "python", "commited": False, "ver_name": f"无法检测版本{base_version}"}
    try:
