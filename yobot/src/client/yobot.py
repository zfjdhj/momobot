...
class Yobot:
    ...
    def __init__(
        self, *, data_path: str, scheduler: AsyncIOScheduler, quart_app: Quart, bot_api: Api, verinfo: str = None
    ):
        ...
        if is_packaged:
            default_config_f_path = os.path.join(sys._MEIPASS, "packedfiles", "default_config.json")
        else:
            default_config_f_path = os.path.join(os.path.dirname(__file__), "packedfiles", "default_config.json")
        with open(default_config_f_path, "r", encoding="utf-8") as config_file:
            self.glo_setting = json.load(config_file)
            # self.glo_setting["public_address"] = "0.0.0.0"
            # print(self.glo_setting)
            # self.glo_setting["public_address"] = "http://192.168.2.100:9025/123/"
            # print(self.glo_setting["public_address"])
        ...
        if not os.path.exists(boss_filepath):
            if is_packaged:
                default_boss_filepath = os.path.join(sys._MEIPASS, "packedfiles", "default_boss.json")
            else:
                default_boss_filepath = os.path.join(os.path.dirname(__file__), "packedfiles", "default_boss.json")
        ...
        if not os.path.exists(pool_filepath):
            if is_packaged:
                default_pool_filepath = os.path.join(sys._MEIPASS, "packedfiles", "default_pool.json")
            else:
                default_pool_filepath = os.path.join(os.path.dirname(__file__), "packedfiles", "default_pool.json")
        ...
        if self.glo_setting["web_gzip"] > 0:
            ...
            async def gzip_response(response):
                ...
                if (
                    ...
                    or len(await response.get_data()) < 1024
        ...
        if not self.glo_setting["public_basepath"].startswith("/"):
            self.glo_setting["public_basepath"] = "/" + self.glo_setting["public_basepath"]
        ...
        if self.glo_setting["update-time"] == "random":
            self.glo_setting["update-time"] = "{:02d}:{:02d}".format(random.randint(2, 4), random.randint(0, 59))
        ...
        if quart_app.secret_key is None:
            quart_app.secret_key = bytes((random.randint(0, 255) for _ in range(16)))
        ...
        @quart_app.route(urljoin(self.glo_setting["public_basepath"], "assets/<path:filename>"), methods=["GET"])
        async def yobot_static(filename):
            accept_encoding = request.headers.get("Accept-Encoding", "")
            origin_file = os.path.join(os.path.dirname(__file__), "public", "static", filename)
            if "gzip" not in accept_encoding.lower() or self.glo_setting["web_gzip"] == 0:
                return await send_file(origin_file)
            gzipped_file = os.path.abspath(
                os.path.join(
            ...
            response.mimetype = mimetypes.guess_type(os.path.basename(origin_file))[0] or "application/octet-stream"
            response.headers["Content-Encoding"] = "gzip"
        ...
        @quart_app.route(urljoin(self.glo_setting["public_basepath"], "output/<path:filename>"), methods=["GET"])
        ...
        # self.glo_setting["public_address"] = str(time.time())
        # print(self.glo_setting)
        kwargs = {
